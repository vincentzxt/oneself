<template>
	<view class="container">
		...
		<view class="footer"><button class="fill" style="background-color: #2d8cf0;" type="primary" @tap="handleAdd">新增</button></view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		setUserLock(userid, isdelete) {
			...
			tokenpost(api.SaveUserStatus, sendData)
				.then(res => {
					if (res.status == 200 && res.data.returnCode == '0000') {
						this.$api.msg('操作成功！');
					} else {
