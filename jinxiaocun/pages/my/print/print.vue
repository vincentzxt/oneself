		...
		<view class="main">
			<scroll-view :scroll-y="true" class="fill">
				<cu-panel>
					...
					<cu-cell title="打印机型号" isLink v-if="reqData.printertype == 1" @tap="handlePrintModel">
						<view slot="footer" style="width:100%;">
					...
					<cu-cell title="设备编号">
						<view class="h50 fc" slot="footer"><input type="text" v-model="reqData.printercode" placeholder-style="color:#c5c8ce" placeholder="请输入设备编号" /></view>
					</cu-cell>
					<cu-cell title="设备密钥">
						<view class="h50 fc" slot="footer"><input type="text" v-model="reqData.printerkey" placeholder-style="color:#c5c8ce" placeholder="请输入设备密钥" /></view>
					</cu-cell>
				</cu-panel>
...
export default {
	...
	onShow() {},
	onLoad(option) {
		uni.$on('printModelChange', val => {
			console.log(val.value);
			this.reqData.printermodel = val.value;
		});
		this.loadData();
	},
	methods: {
		...
		loadData() {
			...
			const sendData = {};
			tokenpost(api.GetFirstActivePrinter, sendData)
				.then(res => {
		...
		Unbind() {
			...
			tokenpost(api.UnBindPrint, sendData)
				.then(res => {
					this.$refs.loading.close();
					if (res.status == 200 && res.data.returnCode == '0000') {
