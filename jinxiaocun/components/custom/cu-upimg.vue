<template name='cu-upimg'>
	<view class="cu-uploader-bd">
		<view class="cu-uploader-files">
			<block v-for="(item,index) in upload_before_list" :key='index'>
				<view class="cu-uploader-file" :class="[item.upload_percent<100?'cu-uploader-file-status':'']" @click="previewImage(index)">
					<image class="cu-uploader-img" :style="'width:'+upload_img_wh+'rpx;height:'+upload_img_wh+'rpx;'" :src="item.path"
					 mode="aspectFill" />
					<view class="cu-img-removeicon right" @click.stop="removeImage(index)" v-show="upimg_move">x</view>
					<view class="cu-loader-filecontent" v-if="item.upload_percent<100">{{item.upload_percent}}%</view>
				</view>
			</block>
			<view v-if="upload_before_list.length<upload_count" hover-class="cu-uploader-hover" class="cu-uploader-inputbox"
...
<style lang="scss">
	...
	.cu-uploader-file-status:before {
		content: " ";
		position: absolute;
		top: 0;
		right: 0;
	...
	.cu-uploader-file:nth-child(4n+0) {
		margin-right: 0;
	}
